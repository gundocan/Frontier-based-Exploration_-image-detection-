...
class Explorer(object):
    def __init__(self):
        ...
        #self.barbie_status_listener = rospy.Subsriber("barbie_status",String, queue_size =1)
        #self.barbie_found = self.barbie_status_listener
        
        #while(self.barbie_found != "FOUND"):
        for i in range (25):
            rospy.wait_for_service('get_closest_frontier')
            req1 =self.service_closest()
            print("the response is",req1)
            #print(self.req1)
            self.resx =(req1.goal_pose.x)
            self.resy =(req1.goal_pose.y)
            self.restheta =(req1.goal_pose.theta)
            ...
            print("I'm finished with iteration:",i)
