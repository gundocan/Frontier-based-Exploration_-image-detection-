...
class PathFollower(object):
    def __init__(self):
        ...
        self.robot_frame = rospy.get_param('robot_frame', 'base_footprint') # base_footprint for simulation, robot for dummy grid
        ...
        self.look_ahead = rospy.get_param('look_ahead_dist', 0.4) # look ahead distance for pure pursuit (m)
        self.const_velocity = rospy.get_param('const_velocity', 0.3) # desired constant velocity for path following
        self.const_angular_rate = rospy.get_param('const_angular_rate', 0.3) #desired constant angular rate for turning on a spot
            ...
            if self.control_law == "turn_and_move":
                ...
                if self.control_law == "pure_pursuit": # cannot be combined with goal heading 
                    if (self.goal_reached == 0):
                        ...
                        line_angle = self.angle_of_line(pose[0], pose[1], goal[0], goal[1])
                        
                        turn_heading_radians = line_angle - pose[2] 
                        
                        #print("Robot_position --> (",round(pose[0]),round(pose[1]),")")
                        #print("Rotation radians :",turn_heading_radians)
                        #print("pose heading :" , pose[2])
                        #print("Rotation degrees:",math.degrees(turn_heading_radians))
                        #print("rotation radyan :",turn_heading_radians)
                        if( 4.1 > abs(turn_heading_radians) > 2.0   and  goal[0] != self.path[-1][0] and goal[1] != self.path[-1][1] ) :
                           self.turnRobot(turn_heading_radians, (6*self.const_angular_rate))
                        if(abs(pose[0]-goal[0]) > 0.46  or  abs(pose[1]-goal[1]) > 0.46):
                           self.turnRobot(turn_heading_radians, (5*self.const_angular_rate))
                       
                        #rospy.logwarn("linetoPoint %.2f", d)
                        #rospy.logwarn("Distance to Look ahead point %.2f ", dl)
                        #print("v vector :", v)
                        radius = (dl**2)/(2*d)
                        #rospy.logwarn("radius %.2f  ", radius)
                        angular_rate = self.const_velocity / radius
                        # Burada angular_rate i 10 ile carpalim, hizli reaksiyon
                        #angular_rate *=  3.0 #5.5 #5.00 #5 #10.0
                        ...
                        if( abs(pose[0]-self.path[-1][0]) < 0.28  and  abs(pose[1]-self.path[-1][1])< 0.28) :
